...
create view Propiedad as

select 
casa,depto,propiedad,moneda,tipo_de_operacion,ubicacion,precio,superficie,superficie_cubierta,descripcion
from 
Propiedad 
inner join Tipo_de_Operacion on a.id_prop = b.id_tipop 
inner join Tipo_de_propiedad on a.id_prop = b.id_tipop
inner join Tipo_de_Moneda  on b.id_tipop = c.id_moneda
inner join Tipo_de_departamento on c.id_moneda = id_dep 
inner join Tipo_de_casa on id_dep = id_casa
...
-- movida mas abajo para que el script pueda correr sin problemas
...
nombre,apellido,cuil,gerente,direccion,fecha_hora,observaciones,tipo,numero,nombre,apellido,codigo_postal,dni,cuil,domicilio,provincia,mail
FROM 
Clientes inner join telefono on a.id_cliente = b.id_tel
inner join Factura on b.id_tel = c.id_factura
inner join Sucursal on c.id_factura = d.id_sucursal 
inner join Vendedor on d.id_factura = e-id_vendedor
...
select ubicacion, precio from Propiedad inner join Tipo_de_propiedad on Propiedad.id_tipoprop = Tipo_de_propiedad.id_tipoprop

--Consulta sobre tipo de propiedades con un valor entre 100.000 y 800.000 dolares

select Tipo_de_propiedades.  from Propiedad inner join Tipo_de_propiedad on Propiedad.id_tipoprop = Tipo_de_propiedad.id_tipoprop inner join Tipo_de_Moneda on Propiedad.id_moneda = Tipo_de_Moneda.id_moneda where (precio between 100000 and 800000)and (moneda like 'Dolar')
